						  ...
						  implements ItemSelectable, ActionListener
{
	...
	final static int hgap = 7;
	final static int vgap = 2;
	final static int hextra = 12;
	final static int vextra = 0;

	//Color SCbackgroundColor = new Color(240,240,240);
	Color SCbackgroundColor = Color.lightGray;
	Color SCfontColor = Color.black;
	Color SCinactivebackgroundColor = Color.lightGray;
	Color SCinactivefontColor = Color.gray;
	
	Font SCfont;
	
    transient ItemListener itemListener = null;

	boolean active = true;				//whether we are showing popupmenu when moused (and doing actions)
	...
	public SophieChoice( String name )
	{
		setName(name);
		pItems = new Vector();
		setBackground( SCbackgroundColor );
		setForeground( SCfontColor );
		SCfont = new Font("Serif",Font.PLAIN,11);
		setFont( SCfont );
	...
	public void paint( Graphics g, int xorg, int yorg )
	{
		int x, y;
		int x0, y0;
		int len;
		...
		//{
		//	System.out.println("Sophies height=="+size.height+" y0=="+y0+" yorg=="+yorg);
		//	oldheight = size.height;
		//}
	
		if( active )
			g.setColor( SCbackgroundColor );
		else
			g.setColor( SCinactivebackgroundColor );
		g.fillRoundRect( xorg+x0, yorg+y0, size.width-1-x0, size.height-3-y0, 6, 6 );
		if( active )
			g.setColor( SCfontColor );
		else
			g.setColor( SCinactivefontColor );
		g.drawRoundRect( xorg+x0, yorg+y0, size.width-1-x0, size.height-3-y0, 6, 6 );

		x = size.width-hextra+1;
		g.drawLine( x, yorg+y0, x, size.height-3+yorg );
		...
		{
			g.setFont( SCfont );
			FontMetrics fm = getFontMetrics( SCfont );
			
			g.drawString( s, xorg+x0+hgap, yorg+y0+fm.getAscent()-1+vgap );
		}
	}
