...
public class Charge extends Component implements GraphElement, Runnable
{
	...
	public Charge( GraphInfo inf, double charge )
	{
		...
		arrow.setForeground( GraphInfo.FORCE_VECTOR_COLOR );
	}
	...
	{
		Fector fec = calcCharge( parent.getComponents(), scale, getOrigin(), this, this.charge );
		fec.setLength( fec.getLength() * 3.5 );
		arrow.setFector(fec);
	}
	...
	static public double calcPotential( Component [] clist, DPoint O )
	{
		...
		for( int i=0; i<len; ++i )
		{
			...
			if( r2 != 0 )
			{
				potential += (ch.charge)/Math.sqrt(10*r2);
	...
	{
		int d = info.fc2 ? GraphInfo.DOTSIZE * getRadius() : 16;
		if( test ) d /= 2;
		return d;
	}
	...
	public void paint( Graphics g )
	{
		...
		if( info.getShowForce() )
			arrow.drawArrow( g, x0, y0 );
