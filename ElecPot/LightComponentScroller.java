...
class LightScrollbar
	...
	implements Runnable
{
  static final boolean custom_slider = true;	//false==almost peer looking (but nicer)
  												//true==draw like the art dept wants (custom)
  ...
  static final int arrow_width_height = 10;	//was 13;
  boolean top_up = true;
  ...
  public void paintSingleBuffer(Graphics g) {
    ...
    g.setColor(getBackground());					//clear old values
    //g.setColor(ElecPot.userbackground);					//clear old values
    g.fill3DRect(0, 0, w+2, h+2, true);
    ...
    switch (orientation) {
      case VERTICAL:
	    ...
	    if( custom_slider )
	    	g.drawLine( (arrow_width_height/2), arrow_width_height, w - arrow_width_height - 2, (arrow_width_height/2) );
      ...
      case HORIZONTAL:
	    ...
	    if( custom_slider )
	    	g.drawLine( arrow_width_height, (arrow_width_height/2), w - arrow_width_height - 2, (arrow_width_height/2) );
	...
	s = h - 2 * arrow_width_height;
	    ...
	    if( !custom_slider )
      	...
      	drawArrowHead(0, bar_xy, g, f, ARROW_DOWN, bottom_up);
      ...
      case HORIZONTAL:
	    ...
	    if( !custom_slider )
      	...
      	drawArrowHead(bar_xy, 1, g, f, ARROW_DOWN, bottom_up);
  ...
  private void drawArrowHead(int left, int top, Graphics g, Color base, int direction, boolean raised) {
	...
	{  	
	  	...
	  	switch (direction) {
	  		case ARROW_DOWN:
				g.drawLine(left + 4, top + 5, left + 8, top + 5);
				g.drawLine(left + 5, top + 6, left + 7, top + 6);
				g.drawLine(left + 6, top + 7, left + 6, top + 7);
				break;
	  		case ARROW_UP:
				g.drawLine(left + 4, bottom - 5, left + 8, bottom - 5);
				g.drawLine(left + 5, bottom - 6, left + 7, bottom - 6);
				g.drawLine(left + 6, bottom - 7, left + 6, bottom - 7);
				break;
	  		case ARROW_LEFT:
				g.drawLine(right - 5, top + 4, right - 5, top + 8);
				g.drawLine(right - 6, top + 5, right - 6, top + 7);
				g.drawLine(right - 7, top + 6, right - 7, top + 6);
				break;
	  		case ARROW_RIGHT:
				g.drawLine(left + 5, top + 4, left + 5, top + 8);
				g.drawLine(left + 6, top + 5, left + 6, top + 7);
				g.drawLine(left + 7, top + 6, left + 7, top + 6);
				break;
	  	}
  	}
  	else
	{  	
	  	...
	  	switch (direction) {
	  		case ARROW_DOWN:
				tri.addPoint(left, top );
				tri.addPoint(right, top );
				tri.addPoint(left+(arrow_width_height/2), bottom );
				break;
	  		case ARROW_UP:
				tri.addPoint(left, bottom );
				tri.addPoint(right, bottom );
				tri.addPoint(left+(arrow_width_height/2), top );
				break;
	  		case ARROW_LEFT:
				tri.addPoint(right, top );
				tri.addPoint(right, bottom );
				tri.addPoint(left, arrow_width_height/2 );
				break;
	  		case ARROW_RIGHT:
				tri.addPoint(left, top );
				tri.addPoint(left, bottom );
				tri.addPoint(right, arrow_width_height/2 );
				break;
	  	}
	  	g.fillPolygon( tri );
  	}
