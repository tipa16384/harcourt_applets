...
public class Graph extends Component
{
	...
	static public final int CONNECT_POINTS	  = 0x0100;
	...
	public void setXWinnow( double xw )
	{
		xWinnow = xw;
		repaint();
	}
	...
	public void setYMinMax( double yMin, double yMax )
	{
		...
		if( minX < 0.0 )
		{
			...
			leftgap = tickFontMetrics.stringWidth(getTickLabel(yMax))+tickSize;
		...
		if( minY < 0.0 )
			...
			bottomgap = tickFontMetrics.getHeight() + tickSize;
	...
	public void removePoint( int idx )
	{
		try
		{
			//System.out.println("removing point "+idx);
	...
	{
		//System.out.println("removing all points");
	...
	public void paint( Graphics g )
	{
		...
		// draw the axes
		{
			...
			if( active(USE_CONTENT_COLOR) )
			{
				...
				g.fillRect( getIX(minX), getIY(Math.pow(10.0,maxY)), ixRange, iyRange );
			}
			...
			iy = (minY <= 0.0) ? getIY(0.0) : getIY(Math.pow(10,minY));
			...
			ix = getIX(0.0);
			Color grid2Color = getBackground().brighter();
			
			for( y = minY; y <= maxY; y += yTick )
			{
				if( !active(SUPPRESS_Y_ZERO) || (y != 0.0) )
				{
					iy = getIY(Math.pow(10.0,y));
					
					if( active(DRAW_Y_GRID) )
					{
						if( y > minY )
						{
							double bval = Math.pow(10.0,y-1);
							double dval = bval;
							bval += dval;
							
							g.setColor( grid2Color );
							
							for( int i=1; i<10; ++i, bval += dval )
							{
								//System.out.println("Plotting "+bval);
								int iiy = getIY(bval);
								g.drawLine( getIX(minX), iiy, getIX(maxX), iiy );
							}
						}
			...
			if( yLabel != null )
			{
				iy = getIY(Math.pow(10,maxY - maxY % yTick));
		...
		// draw the points
		{
			...
			if( active(CONNECT_POINTS) )
			{
				g.setColor( new Color( 0x33, 0x33, 0x99 ) );
				
				for( int ipoint=1; ipoint<len; ++ipoint )
				{
					PointEntry pe = (PointEntry) pointEntries.elementAt(ipoint);
					PointEntry pe0 = (PointEntry) pointEntries.elementAt(ipoint-1);
					double x = pe.getX();
					double y = pe.getY();
					double x0 = pe0.getX();
					double y0 = pe0.getY();
					...
					double pyMin = Math.pow(10,minY);
					double pyMax = Math.pow(10,maxY);
			...
			for( int ipoint=0; ipoint<len; ++ipoint )
			{
				...
				if( x >= xFrom && x <= xTo && y >= pyMin && y <= pyMax )
				{
					g.setColor( color );
					int ix = getIX(x);
					int iy = getIY(y);
					
					//System.out.println("Plotting "+x+","+y+" at "+ix+","+iy);
		...
		// draw the key
		{
			...
			if( active(GRAPH_BORDER) )
			{
				...
				int by = getIY(Math.pow(10,maxY));
				int by0 = getIY(Math.pow(10,minY));
				int byz = getIY(0);
	...
	String getTickLabel( double yval )
	{
		return "1.0E"+(int)yval;
	}
	...
	{
		double logy = Math.log(y)/Math.log(10.0);
		
		int iy = (int) Math.rint( ((logy-maxY)*yRange)/(minY-maxY) );
